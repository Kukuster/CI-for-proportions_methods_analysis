...
def getFileName(method_filename: str, probs: List[float], step: str, numTrials: int, numSamples: int, theme_filename: str = ""):
    return f"2d_{method_filename}_pfrom{probs[0]}_pto{probs[-1]}_pstep{step}_trials{numTrials}_samples{numSamples}{theme_filename}.png"
...
def dump_coverage_to_file(coverage: np.ndarray):
    frm = pd.DataFrame(coverage)
    frm.to_csv("/path/to/file.csv", header=None, index=None)
...
def plot_coverage(probs: Iterable[float], coverage: np.ndarray, conflevel: float, title: str, xlabel: str, ylabel: str):
    ...
    max_num_xticks = 10
    max_num_yticks = 20
    # if len(probs) <= max_num_ticks:
    #     period = 1
    # else:

    xperiod = math.ceil(len(probs)/max_num_xticks)
    yperiod = math.ceil(len(probs)/max_num_yticks)

    xperiodic_probs = probs[::xperiod]
    yperiodic_probs = probs[::yperiod]
    print(f"xperiodic_probs = {xperiodic_probs}")
    print(f"yperiodic_probs = {yperiodic_probs}")
    ax.set_xticklabels([0.0] + xperiodic_probs)
    ax.set_yticklabels([0.0] + yperiodic_probs)
    ax.xaxis.set_major_locator(ticker.MultipleLocator(xperiod))
    ax.yaxis.set_major_locator(ticker.MultipleLocator(yperiod))

    # plt.xticks(periodic_probs_C)
    # plt.yticks(periodic_probs_C)
    # print(f"periodic_probs_C = {periodic_probs_C}")
    # # ax.set_xticks(np.arange(len(periodic_probs)))
    # ax.set_yticks(np.arange(len(periodic_probs)))
    # # ... and label them with the respective list entries
    # ax.set_xticklabels(periodic_probs)
    # ax.set_yticklabels(periodic_probs)
...
for (theme, theme_filename) in [
    ...
    # plt.show()
    plt.savefig(
        f"2d_{method_filename}_pfrom{probs[0]}_pto{probs[-1]}_pstep{step}_trials{numTrials}_samples{numSamples}{theme_filename}.png")
